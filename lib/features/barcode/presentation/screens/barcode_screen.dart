...
class _BarcodeScreenState extends ConsumerState<BarcodeScreen>
  ...
  void _simulateBarcodeScan(List<Medicine> medicines) {
    final medicinesWithBarcode = medicines
        .where((m) => m.barcode != null && m.barcode!.isNotEmpty)
        .toList();
    if (medicinesWithBarcode.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'No medicines have barcodes. Register one in Stock first.',
          ),
                    ...
                    children: [
                      ...
                      SizedBox(
                        ...
                        child: Container(
                          ...
                          child: ElevatedButton.icon(
                            onPressed: () {
                              if (_generatorTextController.text
                                  .trim()
                                  .isEmpty) {
                                ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(
                                    content: Text(
                                      'Please enter code text to generate.',
                                    ),
                                    backgroundColor: Color(0xFFEF4444),
                                    behavior: SnackBarBehavior.floating,
                                  ),
  ...
  Widget _buildLookupResultCard(
    ...
    final manufacturerName =
        mfgMap[med.manufacturerId] ?? 'Unknown Manufacturer';
