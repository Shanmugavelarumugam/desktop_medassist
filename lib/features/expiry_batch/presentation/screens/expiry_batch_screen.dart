...
class _ExpiryBatchScreenState extends ConsumerState<ExpiryBatchScreen> {
  ...
  void initState() {
    ...
    _searchController.addListener(() {
      ref.read(expiryBatchNotifierProvider.notifier).setSearchQuery(_searchController.text);
  ...
  void _showActionConfirmDialog({
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(content, style: const TextStyle(color: _softGrey, fontSize: 14, height: 1.5)),
        actions: [
          TextButton(
            onPressed: () => Navigator.of(ctx).pop(),
            child: const Text('Cancel', style: TextStyle(color: _softGrey, fontWeight: FontWeight.bold)),
          ),
          ElevatedButton(
            ...
            onPressed: () async {
              ...
              ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                content: Text(success ? 'Action completed successfully.' : 'Action failed. Please try again.'),
  ...
  Widget build(BuildContext context) {
    ...
    for (final b in state.batches) {
      ...
      if (isQ) { quarantinedCount++; continue; }
      try {
    ...
    final filtered = state.batches.where((b) {
      ...
      if (query.isNotEmpty &&
          ...
          !batchNo.contains(query)) { return false; }
      ...
      try {
        ...
        if (!isExp) isNear = exp.difference(now).inDays <= 90;
      } catch (_) {}

      switch (state.filterStatus) {
        case 'Expired':     return isExp  && !isQ && !isR;
        case 'Near Expiry': return isNear && !isQ && !isR;
        case 'Quarantined': return isQ;
        case 'Recalled':    return isR;
        case 'Active':      return !isExp && !isNear && !isQ && !isR;
        default:            return true;
      }
    }).toList();
                ...
                Column(
                  ...
                  children: const [
                    ...
                    Text(
                      ...
                      style: TextStyle(color: _softGrey, fontSize: 14, fontWeight: FontWeight.w500),
                    ),
                  ],
                ),
                ElevatedButton.icon(
                  onPressed: () => ref.read(expiryBatchNotifierProvider.notifier).loadBatches(),
                  icon: const Icon(Icons.refresh_rounded, size: 18),
                  label: const Text('Refresh Batches', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
                  style: ElevatedButton.styleFrom(
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      Expanded(child: _buildStatCard('Total Batches',      totalBatches.toString(),      Icons.layers_outlined,           const Color(0xFF3B82F6))),
                      const SizedBox(width: 16),
                      Expanded(child: _buildStatCard('Expired',            expiredCount.toString(),       Icons.error_outline_rounded,     const Color(0xFFEF4444))),
                      const SizedBox(width: 16),
                      Expanded(child: _buildStatCard('Expiring Soon',      nearExpiryCount.toString(),    Icons.warning_amber_rounded,     const Color(0xFFF59E0B))),
                      const SizedBox(width: 16),
                      Expanded(child: _buildStatCard('Quarantined',        quarantinedCount.toString(),   Icons.health_and_safety_outlined, const Color(0xFF6366F1))),
                    ],
                  ),
                    ...
                    child: Row(
                      children: [
                        ...
                        Expanded(
                          ...
                          child: TextField(
                            ...
                            decoration: InputDecoration(
                              ...
                              hintStyle: TextStyle(color: _softGrey.withValues(alpha: 0.7), fontSize: 14),
                              prefixIcon: Icon(Icons.search_rounded, color: _softGrey.withValues(alpha: 0.7), size: 20),
                        ...
                        _FilterTabs(
                          ...
                          onChanged: (s) {
                            ref.read(expiryBatchNotifierProvider.notifier).setFilterStatus(s);
                        ...
                        Container(
                          ...
                          child: Row(
                            children: const [
                              ...
                              Expanded(flex: 2, child: _HeaderText('BATCH NUMBER')),
                              ...
                              SizedBox(width: 80, child: _HeaderText('ACTION', alignRight: true)),
                            ],
                          ),
                        ),
                            ...
                            : filtered.isEmpty
                                ...
                                : ListView.separated(
                                    ...
                                    itemBuilder: (context, index) {
                                      ...
                                      // ── Status logic ──────────────────────
                                      final isQ = batch.status == 'QUARANTINED' ||
                                          batch.status == 'quarantined';
                                      final isR = batch.status == 'RECALLED' ||
                                      ...
                                      try {
                                        formattedExpiry = DateFormat('MMM yyyy')
                                                    ...
                                                    onSelected: (val) {
                                                      if (val == 'quarantine') {
                                                        _showActionConfirmDialog(
                                                          ...
                                                          content:     'Are you sure you want to quarantine batch "${batch.batchNumber}"? This will freeze it and make it unavailable for billing.',
                                                          actionColor: const Color(0xFFF59E0B),
                                                          actionIcon:  Icons.health_and_safety_outlined,
                                                          actionLabel: 'Quarantine',
                                                          onConfirm:   () => ref.read(expiryBatchNotifierProvider.notifier).quarantineBatch(batch.id),
                                                        );
                                                      } else if (val == 'release') {
                                                        _showActionConfirmDialog(
                                                          title:       'Release Batch',
                                                          content:     'Release batch "${batch.batchNumber}" so it becomes active for billing again?',
                                                          actionColor: _teal,
                                                          actionIcon:  Icons.verified_outlined,
                                                          actionLabel: 'Release',
                                                          onConfirm:   () => ref.read(expiryBatchNotifierProvider.notifier).releaseBatch(batch.id),
                                                        );
                                                      } else if (val == 'recall') {
                                                        _showActionConfirmDialog(
                                                          title:       'Recall Batch',
                                                          content:     'Are you sure you want to recall batch "${batch.batchNumber}"? This will alert users to pull stock from shelves.',
                                                          actionColor: const Color(0xFFEF4444),
                                                          actionIcon:  Icons.assignment_return_outlined,
                                                          actionLabel: 'Recall',
                                                          onConfirm:   () => ref.read(expiryBatchNotifierProvider.notifier).recallBatch(batch.id),
                                                        );
                                                      }
                                                    },
                                                    itemBuilder: (_) => [
                                                      if (!isQ)
                                                        _menuItem('quarantine', Icons.health_and_safety_outlined, 'Quarantine Batch', const Color(0xFFF59E0B)),
                                                      if (isQ)
                                                        _menuItem('release', Icons.verified_outlined, 'Release Batch', _teal),
                                                      if (!isR)
                                                        _menuItem('recall', Icons.assignment_return_outlined, 'Recall Batch', const Color(0xFFEF4444)),
                                                    ],
                                                  ),
                                                ),
                                              ),
                            ...
                            child: Row(
                              ...
                              children: [
                                Text(
                                  ...
                                  style: const TextStyle(color: _softGrey, fontSize: 13, fontWeight: FontWeight.w500),
                                ),
                                Row(
                                  children: [
                                    OutlinedButton(
                                      ...
                                      child: const Text('Previous', style: TextStyle(color: _textDark)),
                                    ),
                                    const SizedBox(width: 12),
                                    Container(
                                    ...
                                    OutlinedButton(
                                      ...
                                      child: const Text('Next', style: TextStyle(color: _textDark)),
                                    ),
                                  ],
                                ),
                              ],
    ...
    return PopupMenuItem<String>(
      ...
      child: Row(
        children: [
          ...
          Text(
            label,
            style: TextStyle(fontSize: 13, color: _textDark, fontWeight: FontWeight.w500),
          ),
        ],
      ),
    );
    ...
    return Padding(
      ...
      child: Column(
        children: [
          ...
          const Text('No batches found', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Color(0xFF334155))),
          const SizedBox(height: 6),
          const Text('Try adjusting your search or filter selection.', style: TextStyle(color: _softGrey, fontSize: 13)),
        ],
      ),
    );
  }
...
class _FilterTabs extends StatelessWidget {
  ...
  static const _filters = ['All', 'Active', 'Near Expiry', 'Expired', 'Quarantined', 'Recalled'];
